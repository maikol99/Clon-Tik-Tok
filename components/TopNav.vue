    ...
    >
        ...
        >
            ...
            <div class="flex items-center justify-end gap-3 min-w-[275px] max-w-[320px] w-full">
                ...
                <!-- Comprobar si $userStore y su propiedad id existen -->
                    ...
                    <div class="relative">
                        ...
                        >
                            <img 
                                class="rounded-full" 
                                width="33" 
                                :src="$userStore?.image" <!-- Verificar si la imagen existe -->
                            >
                        </button>
                        ...
                        >
                            <NuxtLink 
                                :to="`/profile/${$userStore?.id}`" <!-- Verificar si el id existe -->
...
<script setup>
const { $userStore = {}, $generalStore } = useNuxtApp() // Inicializar $userStore
...
onMounted(() => {
    document.addEventListener('mouseup', function(e) {
        ...
        if (popupMenu && !popupMenu.contains(e.target)) { // Verificar que PopupMenu existe
...
const isLoggedIn = () => {
    if ($userStore?.id) { // Verificar si el id existe
        router.push('/upload')
    } else {
